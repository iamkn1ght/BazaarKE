...
**Status:** 🟠 DESIGN ASK — no `helpan-unique-accessories-v1` agent exists; nothing ships this phase. This scopes the agent + credentials + the delegated-authority posture.
...
| Dual-role dispatch target `app/api/agent/checkout/route.ts` | UA eng | ⏳ design |
| `initiated_by:"agent"` + `agent_id` on all agent-initiated rail calls | UA eng | ⏳ design |
| Reject dispatches after `AUTHORITY_REVOKED` (≤5s) | UA eng | ⏳ design |
| Unconfigured-dispatcher fallback (env unset → `failed/TARGET_RAIL_UNCONFIGURED`, not a crash) | UA eng | ⏳ design |
